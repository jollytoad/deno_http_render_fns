...
export function isIterable<T>(value: unknown | Iterable<T>): value is Iterable<T> {
  return typeof (value as Iterable<T>)?.[Symbol.iterator] === "function"
}

export function isAsyncIterable<T>(value: unknown | AsyncIterable<T>): value is AsyncIterable<T> {
